...
export default async function addDetailsToPdf(data, user, status) {

  let pdfPath;
  if (status) {

    const url = data?.file;
    const restOfPath = url.substring(url.indexOf("/contractUpload/") + "/contractUpload/".length);
    pdfPath = `./contractUpload/${restOfPath}`; // Provide the path to your existing PDF

  } else {
    pdfPath = './contractUpload/contract.pdf'; // Provide the path to your existing PDF
  ...
  try {
    ...
    console.log('Details added to PDF successfully.');
    return path
  } catch (error) {
